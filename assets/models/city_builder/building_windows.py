...
def create_militech_windows(militech_objects, materials, interior_materials):
    ...
    # --- DELETED THESE LINES ---
    # bm = bmesh.from_edit_mesh(bpy.context.active_object.data) # REMOVED
    # bm.verts.ensure_lookup_table()                          # REMOVED
    # --- END DELETED LINES ---
    ...
    # Create a subcollection for window objects
    if "MilitechArmory_Windows" not in bpy.data.collections:
        windows_collection = bpy.data.collections.new("MilitechArmory_Windows")
        collection.children.link(windows_collection)
    else:
        windows_collection = bpy.data.collections["MilitechArmory_Windows"]
    ...
    for i in range(window_count):
        ...
        bpy.ops.mesh.primitive_cube_add(
            size=1.0,
            enter_editmode=True, # Enters Edit Mode for the new cube
            align='WORLD',
            location=(window_x, window_y, window_z)
        )
        window_frame = bpy.context.active_object
        ...
        # Edit the window frame
        # Now this is correct because the cube is in Edit Mode
        bm = bmesh.from_edit_mesh(window_frame.data)
        ...
        # Assign material
        # Ensure the material key exists before appending
        if "Militech_Interior" in interior_materials:
             window_frame.data.materials.append(interior_materials["Militech_Interior"])
        else:
             print(f"Warning: Material 'Militech_Interior' not found for {window_frame.name}")
        ...
        bpy.ops.mesh.primitive_cube_add(
            size=1.0,
            enter_editmode=True, # Enter Edit Mode for the accent cube
            align='WORLD',
            location=(window_x, window_y + 0.05, window_z)
        )
        ...
        bm = bmesh.from_edit_mesh(window_accent.data) # Get BMesh for accent cube
        ...
        # Make it hollow (This part might need adjustment depending on desired look)
        # Ensure lookup tables are fresh *after* getting the BMesh
        bm.verts.ensure_lookup_table()
        bm.faces.ensure_lookup_table()

        # Store original verts before adding new ones
        original_verts = list(bm.verts)

        inner_verts = []
        for j in range(len(original_verts)):
             v_orig = original_verts[j]
             # Create new vertex slightly inset
             inner_vert = bm.verts.new((
                 0.9 * v_orig.co.x,
                 1.1 * v_orig.co.y,  # Make it go through? Check if this Y scale is intended
                 0.9 * v_orig.co.z
             ))
        ...
        # --- Potential Issue Here: Creating faces for inner cutout ---
        # The previous code attempted to create faces using indices based on the original 8 cube verts.
        # This BMesh approach needs careful indexing after adding verts.
        # A simpler approach might be to use bmesh.ops.inset or Solidify Modifier.
        # For now, let's try a basic Solidify approach conceptually (won't add modifier here, just illustrate logic):
        # Option 1: Keep it simple - don't make it hollow yet if BMesh face creation is complex.
        # Option 2: Try fixing face creation (more complex):
        #   Need to identify corresponding inner/outer verts correctly to bridge them.
        # Option 3: Use Solidify Modifier (applied later or kept live) - often easiest.

        # Let's comment out the face creation for now to avoid further BMesh errors,
        # you can refine this hollowness later.
        # for j in range(4):
        #     # This indexing is likely wrong after adding vertices
        #     bm.faces.new([
        #         inner_verts[j],
        #         inner_verts[(j+1)%4],
        #         inner_verts[(j+1)%4+4],
        ...
        if "Militech_Accent" in materials:
            window_accent.data.materials.append(materials["Militech_Accent"])
             ...
             print(f"Warning: Material 'Militech_Accent' not found for {window_accent.name}")
        ...
        bpy.ops.mesh.primitive_cube_add(
            size=1.0,
            enter_editmode=True, # Enter Edit Mode for glass cube
            align='WORLD',
            location=(window_x, window_y + 0.1, window_z)
        )
        window_glass = bpy.context.active_object
        ...
        # Edit the window glass
        bm = bmesh.from_edit_mesh(window_glass.data) # Get BMesh for glass cube
        ...
        # Ensure Transmission is enabled if using Eevee/Cycles for transparency
        principled.inputs['Transmission Weight'].default_value = 0.7 # Renamed in Blender 4.x
    ...
    # Move all objects to the windows collection
    current_scene_collection = bpy.context.scene.collection
    for obj in window_objects:
         # Ensure the object is not already in the target collection
         is_already_linked = False
         for coll in obj.users_collection:
             if coll == windows_collection:
                 is_already_linked = True
                 break
             # Optional: Unlink from the scene's top-level collection if desired
             if coll == current_scene_collection:
                 coll.objects.unlink(obj)

         if not is_already_linked:
             windows_collection.objects.link(obj) # Link to target collection
...
def implement_building_windows(building_objects, materials, interior_materials):
    ...
    if "militech_armory" in building_objects:
        windows["MilitechArmory_Windows"] = create_militech_windows(
